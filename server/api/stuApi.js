...
router.post("/Entry", function(req, res) {
  ...
  let begin_place = obj.substring(
    obj.indexOf('begin_place":') + 14,
    obj.indexOf("end_place") - 3
  );
  let end_place = obj.substring(
    obj.indexOf('end_place":') + 12,
    obj.indexOf("reason") - 3
  );
  let description = obj.substring(
    obj.indexOf('reason":') + 8,
    obj.indexOf("description") - 2
  );
  let reason = obj.substring(
    obj.indexOf('description":') + 14,
    obj.indexOf("}") - 1
  );
  console.log(idstudents, begin_place, end_place, description, reason);
  conn.query(
    sql,
    [idstudents, begin_place, end_place, description, reason],
    function(err, result) {
      if (err) {
        console.log(err);
        return res.send({
...
router.post("/addfever", (req, res) => {
  ...
  console.log(obj);
  let idstudents = obj.substring(
    obj.indexOf("idstudents") + 15,
    obj.indexOf("atschool") - 5
  );
  console.log(idstudents);
  let atschools = obj.substring(
    obj.indexOf("atschool") + 11,
    obj.indexOf("fever") - 3
  );
